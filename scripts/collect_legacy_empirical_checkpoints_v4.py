#!/usr/bin/env python3
"""Teacher-force legacy-v4 Dense tokens and fork BF16 forced-answer branches from cropped KV."""
...
from src.final_paper_protocol import BOUNDARY, canonical_fingerprint, checkpoint_schedules
...
def main() -> None:
    ...
    for local_index, source_path in enumerate(dense_files):
        ...
        expected_config = canonical_fingerprint(config)
        if source.get("config_fingerprint") != expected_config:
            raise ValueError(f"Dense/config fingerprint mismatch for {problem_id}")
        if source.get("dtype") != "bfloat16":
            raise ValueError(f"legacy-v4 Dense must be bfloat16 for {problem_id}")
        ...
        if union:
            ...
            for union_index, checkpoint in enumerate(union):
                branch = branch_from_legacy_cache(
                    ...
                    seed=int(source["example_seed"]) + int(checkpoint) * 7919,
                )
                ...
                row = {
                    ...
                    "split": args.split,
                    "seed": args.seed,
                    "branch_seed": int(source["example_seed"]) + int(checkpoint) * 7919,
