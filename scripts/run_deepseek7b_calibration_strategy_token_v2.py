#!/usr/bin/env python3
"""Compare token-optimized calibration policies on frozen DeepSeek-7B v2 trajectories.
...
or certify a threshold; MATH-500 and AIME are never used for selection.  Every
feasible-threshold choice maximizes calibration token reduction; wall time is
neither a selection objective nor a reported metric.
"""
...
from src.legacy_empirical_probe_normalized_v1 import simulate_policy, target_values  # noqa: E402
...
def replay_candidate_curve(frame, scores, direction: str, grid, fallbacks):
    ...
    matrices: dict[str, list[np.ndarray]] = {
        key: [] for key in ("success", "lost", "tokens", "dense_tokens", "coverage")
    }
...
def most_token_efficient_index(curve: list[dict[str, Any]], candidates: list[int]) -> int:
    return min(
        candidates,
        key=lambda index: (
...
def select_bonferroni(curve, lost_counts, alpha: float, delta: float):
    ...
    index = most_token_efficient_index(curve, feasible)
...
def select_empirical_budget_token(
    curve: list[dict[str, Any]], budget: int
) -> dict[str, Any]:
    feasible = [
        index
    ...
    index = most_token_efficient_index(curve, feasible)
    selected = dict(curve[index])
    selected["budget_B"] = int(budget)
    selected["selection_objective"] = "maximize_calibration_token_reduction"
    return selected
...
def bootstrap_selection(
    ...
    mean_dense_tokens = weights @ matrices["dense_tokens"].T / n
    token_reduction = 1.0 - mean_tokens / mean_dense_tokens
    coverage = weights @ matrices["coverage"].T / n
    outputs = []

    def choose_most_token_efficient(
        feasible: np.ndarray,
        ...
        return min(
            ...
            key=lambda index: (
                -float(reduction[index]),
                -float(current_coverage[index]),
                index,
            ),
        )
    ...
    for alpha in alphas:
        ...
        for sample in range(resamples):
            selections["bonferroni_cp"].append(
                choose_most_token_efficient(
                    np.r_[True, lost[sample, 1:] <= allowed_bonf],
                    token_reduction[sample], coverage[sample],
                )
            )
            ...
            selections["fixed_sequence_ltt"].append(
                choose_most_token_efficient(
                    mask, token_reduction[sample], coverage[sample]
                )
            )
            selections["trajectory_first_failure_conformal"].append(
                choose_most_token_efficient(
                    np.r_[True, critical[sample, 1:] <= allowed_tffc],
                    token_reduction[sample], coverage[sample],
                )
            )
            ...
            mask = np.arange(len(curve)) <= end
            selections["trajectory_envelope_ltt"].append(
                choose_most_token_efficient(
                    mask, token_reduction[sample], coverage[sample]
                )
            )
...
def main() -> None:
    ...
    for method in METHODS:
        ...
        # Re-evaluate the historical empirical-B risk constraints under the new,
        # token-only selection objective.
        for budget in EMPIRICAL_BUDGETS:
            calibration = select_empirical_budget_token(
                report["calibration"]["curve"], int(budget)
            )
            for dataset in DATASETS:
                test, ci = evaluate_test(
                    dataset,
                ...
                rows.append(
                    result_row(
                        ...
                        budget=int(budget),
                        selection={
                            "historical_calibration_grid": True,
                            "selection_objective": "maximize_calibration_token_reduction",
                        },
                        test_ci=ci,
                    )
    ...
    recommendation_payload = {
        ...
        "rationale": (
            ...
            "calibration token reduction over the predeclared alpha grid, then bootstrap stability. "
    ...
    summary_lines = [
        "# DeepSeek-7B Token-Objective Calibration Strategy Study",
        "",
        f"Recommended primary calibrator: **{chosen_family}**",
        "",
        "Threshold selection maximized calibration token reduction under the declared risk constraint. ",
        "Wall time was neither used for selection nor reported as a cost metric. ",
        "MATH-500 and AIME were not used to choose the strategy.",
        "",
    ...
    audit = {
        ...
        "threshold_selection_objective": "maximize_calibration_token_reduction",
        "reported_cost_metric": "token_reduction",
        "wall_time_used_for_selection": False,
