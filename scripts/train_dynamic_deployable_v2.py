#!/usr/bin/env python3
"""训练不读取未来 sentence 长度的可部署动态最优停止器。"""
...
from src.dynamic_optimal_stopping_v1 import DYNAMIC_FEATURE_KINDS
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=(8, 20, 35),
        default=20,
        help="Zero-based Qwen3 decoder block used for the checkpoint state.",
    )
    parser.add_argument(
        "--variant",
    ...
    if args.variant in {"full", "no_trajectory"}:
        value_targets, oracle_state_values = recursive_q_continue_targets(
            train_frame,
        ...
        value_target_mode = "recursive_bellman_q_continue_includes_incremental_cost"
    elif args.variant == "one_step_value":
        value_targets = one_step_q_continue_targets(
            train_frame,
            stop_probability["probe_train"],
            risk_probability["probe_train"],
            lambdas,
        ...
        value_target_mode = "one_step_q_continue_includes_incremental_cost"
    else:
        value_targets = dense_endpoint_q_continue_targets(
            train_frame, lambdas, cost_unit_tokens=cost_unit
        )
        oracle_state_values = value_targets
        value_target_mode = "dense_endpoint_q_continue_includes_remaining_cost"
    value_model = ContinuationQValueBank(len(candidates)).to(device)
    ...
    q_continue_values = {
        split: predict_q_values(value_model, representation, device)
    ...
    for candidate in candidates:
        ...
        row = simulate_deployable_dynamic_policy(
            ...
            q_continue_values["calibration"][:, index],
    ...
    for index in sorted(selected_indices):
        ...
        summary, records = strip_records(simulate_deployable_dynamic_policy(
            frames["heldout"],
            stop_probability["heldout"],
            risk_probability["heldout"],
            q_continue_values["heldout"][:, index],
            mu_value=candidate["mu"],
    ...
    dense_heldout_summary, dense_heldout_records = strip_records(simulate_deployable_dynamic_policy(
        frames["heldout"],
        stop_probability["heldout"],
        risk_probability["heldout"],
    ...
    run_spec = {
        ...
        "architecture": {
            ...
            "heads": ["stop_correctness", "lost_correct_risk", f"q_continue_bank_{len(candidates)}"],
        },
        "feature_kind": args.feature_kind,
        "feature_width": feature_width,
        "layer_zero_based": args.layer,
        "decision": "stop iff p_stop - mu*p_lost_correct >= Q_continue_hat(z_t)",
        "online_observability": "current z_t only; no next checkpoint/dense remainder",
        "q_continue_semantics": "E[-lambda*delta_t_next/4096 + V(z_next) | z_t]",
    ...
    payload = {
        ...
        "candidate_feasibility": candidate_feasibility_counts(
            calibration_curve,
            dense_calibration["dense_accuracy"],
            epsilon,
    ...
    atomic_torch_save({
        ...
        "q_continue_values": {split: torch.from_numpy(value.astype(np.float32)) for split, value in q_continue_values.items()},
