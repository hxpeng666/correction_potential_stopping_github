...
FEATURES = tuple(DYNAMIC_FEATURE_KINDS)
...
def main() -> None:
    ...
    parser.add_argument("--main-feature",default=MAIN_FEATURE)
    ...
    main_feature=str(args.main_feature)
    if main_feature not in features:
        raise ValueError("main-feature必须包含在features中")
    ...
    for dataset in ("gsm8k","mmlu_pro"):
        ...
        main_probe=args.feature_root/main_feature/dataset/"probe"
        main_records=torch.load(main_probe/"policy_records.pt",map_location="cpu",weights_only=False)["records"]
        any_records=next(iter(main_records["empirical_B"].values()))
        ...
        for family,key in (("empirical_B","4"),("formal_alpha","0.02")):
            reference=methods[(main_feature,family,key)]
            ...
            for metric,samples in reference_boot.items():
                ci_rows.append({"dataset":dataset,"feature":main_feature,"family":family,"key":key,"metric":metric,
            ...
            for feature in features:
                ...
                for metric,samples in feature_boot.items():
                    ...
                    bootstrap_rows.append({
                        "dataset":dataset,"feature":feature,"family":family,"key":key,
                        "comparison":f"feature_minus_{main_feature}","metric":metric,
    ...
    report=[
        "# 当前可部署动态主方法的特征全面消融", "",
        "所有变体共享FP16公共缓存、三头动态结构、trajectory protection、递归Bellman Q_continue、训练预算、内部fit/validation、calibration候选与heldout；仅输入列发生变化。", "",
        "所有部署动作只使用当前z_t，不读取下一checkpoint、未来entropy、未来正确性或Dense剩余长度。未来句长仅进入离线Bellman监督目标。", "",
        f"主参考为{main_feature}。主诊断为经验B=4，formal alpha=2%为补充。精简标准在运行前固定为：两个数据集上相对主参考accuracy不低于-0.5pp、token reduction不低于-2pp、W→C不增加超过2题。该标准只用于解释，不据此重新调test策略。", "",
    ]
    for dataset in ("gsm8k","mmlu_pro"):
    ...
    atomic_json({
        ...
        "features":list(features),"main_feature":main_feature,"datasets":["gsm8k","mmlu_pro"],
        "bootstrap_replicates":int(config["statistics"]["bootstrap_replicates"]),
        "heldout_used_for_selection":False,"future_fields_used_by_action":False,
