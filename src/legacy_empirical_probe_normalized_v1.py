"""Legacy empirical-v4 probe features, losses, first-hit replay, and calibration."""
...
def fit_validation_problem_ids(
    frame: pd.DataFrame,
    dataset: str,
    *,
    fraction: float = 0.8,
    seed: int = 0,
    additional_problem_ids: Iterable[str] = (),
) -> tuple[set[str], set[str]]:
    """Exact legacy problem-level 80/20 split using numpy.default_rng(0)."""
    del dataset
    ids = np.asarray(
        sorted(
            set(frame.problem_id.astype(str).unique())
            | {str(value) for value in additional_problem_ids}
        ),
        dtype=object,
    )
    ordered = np.random.default_rng(seed).permutation(ids)
    cut = max(1, min(len(ordered) - 1, int(np.floor(fraction * len(ordered)))))
    fit_ids = set(str(value) for value in ordered[:cut])
    validation_ids = set(str(value) for value in ordered[cut:])
...
def fit_validation_masks(
    ...
    *,
    ...
    validation = frame.problem_id.astype(str).isin(validation_ids).to_numpy()
    if not fit.any() or not validation.any():
        raise ValueError("fit/validation checkpoint rows are empty")
...
def correction_loss(
    ...
    *,
    ...
    """Weighted point BCE plus optional W->C trajectory protection.

    ``normalize_by_count=False`` preserves the historical unnormalized soft-min.
    ``True`` uses log-mean-exp so equal dangerous logits are invariant to the
    number of dangerous checkpoints in a trajectory.
    """
    if beta <= 0:
        raise ValueError(f"beta must be positive, got {beta}")
    if trajectory_weight < 0:
        raise ValueError(
            f"trajectory_weight must be non-negative, got {trajectory_weight}"
        )
    ...
    if trajectory:
        for start, end in zip(offsets[:-1], offsets[1:]):
            ...
            if dangerous.any():
                ...
                log_aggregate = torch.logsumexp(-values / beta, dim=0)
                if normalize_by_count:
                    log_aggregate = log_aggregate - math.log(values.numel())
                soft_minimum = -beta * log_aggregate
    ...
    return point + trajectory_weight * trajectory_value, point, trajectory_value
...
def threshold_grid(
    ...
    """Return 101 legacy score quantiles plus an explicit no-stop sentinel."""
    if grid_size < 2:
        raise ValueError("threshold grid must contain at least two values")
    quantiles = np.quantile(scores, np.linspace(0.0, 1.0, grid_size))
...
def calibration_curve(
    ...
    *,
    ...
    curve = []
    for index, (threshold, no_stop) in enumerate(grid):
        row = simulate_policy(
            frame,
            scores,
            direction,
...
def select_empirical_budget(curve: list[dict[str, Any]], budget: int) -> dict[str, Any]:
...
def calibrate_policies(
    ...
    *,
    ...
    return {
        ...
        "curve": curve,
        "empirical_B": {
            str(value): select_empirical_budget(curve, value)
            for value in empirical_budgets
        },
        "coverage": {
